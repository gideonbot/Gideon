...
export async function Handle(message: Message): Promise<void | Message> {
    ...
    LBG(message.client, message.guild); //check if guild is blacklisted, if yes, leave
    if (message.channel.id === currentguild.chatchnl && !message.editedAt && !message.content.startsWith('^')) return Chat(message.client, message);

    Ads(message.client, message); //check for foreign invites
    ABM(message.client, message); //apply content filter
    NameCheck(message.client, message.member, null); //check nicknames & usernames
    CVM(message.client, message); //apply crossover mode if enabled
