...
export function InitDB(gideon: Client): void {
	...
	if (!guilddb['count(*)']) {
		sql.prepare(
			'CREATE TABLE guilds (guild TEXT PRIMARY KEY, cvmval BIT, abmval BIT, eastereggs BIT, blacklist BIT, chatchnl TEXT, gpd BIT);'
		).run();
	...
	gideon.setGuild = sql.prepare(
		'INSERT OR REPLACE INTO guilds (guild, cvmval, abmval, eastereggs, blacklist, chatchnl, gpd) VALUES (@guild, @cvmval, @abmval, @eastereggs, @blacklist, @chatchnl, @gpd);'
	);
