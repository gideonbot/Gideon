...
export async function run(interaction: CommandInteraction): Promise<unknown> {
	...
	let chosenfilter: (x: gideonapi.AviInfo) => boolean = () => {
		return true;
	};
	let tries = 3;
	let points = 0;
	...
	if (interaction.user.guessing) {
		return interaction.editReply({ embeds: [new MessageEmbed().setTitle('A guessing game is already running!')] });
	}
	...
	try {
		...
		bcollector.on('end', async () => interaction.editReply('The game has ended. You can start a new one.'));

		// @ts-ignore djs types fuckery
		collector.on('collect', async (message) => {
			const similarity = stringSimilarity.compareTwoStrings(
				game.ep_name.toLowerCase().replace(/\s/g, ''),
			...
			await interaction.editReply({ embeds: [incorrectembed], components: [new MessageActionRow().addComponents(buttons)] });
		});
