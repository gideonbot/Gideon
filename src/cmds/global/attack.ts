...
export async function run(interaction: CommandInteraction): Promise<void> {
    ...
    if (user?.id === auth.id || user?.id === interaction.client.user?.id) return interaction.reply({embeds: [new MessageEmbed().setTitle('My protocols forbid any kind of self-harm!')]});
    ...
    return interaction.reply({embeds: [new MessageEmbed().setDescription(`**${attack?.emote}${auth} ${attack?.text}${attack?.emote}**\n\n${attack?.desc}`).setImage(attack?.attackgif as string)]});
}
