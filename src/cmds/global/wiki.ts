...
export async function run(interaction: CommandInteraction, gideon: SapphireClient): Promise<void> {
	...
	const body = (await fetchJSON(api)) as WikiResult;

	// new wikis do some weird stuff, therefore the actual result is the 2nd element
	const index = Object.values(wikis).indexOf(wiki);
	const article = Object.values(body.items)[
		index === 1 || index === 3 ? 1 : 0 || index === 5 ? 1 : 0 || index === 6 ? 1 : 0 || index === 8 ? 1 : 0
	];

	if (!article)
		return interaction.reply({
			content: `There was no result for ${search_term} on the ${wiki.title} Wiki!\nPay attention to capitalization and spelling or search a different wiki!`,
			ephemeral: true
		});
	if (Object.keys(body.items).length < 1)
		return interaction.reply({
			content: `There was no result for ${search_term} on the ${wiki.title} Wiki!\nPay attention to capitalization and spelling or search a different wiki!`,
			ephemeral: true
		});
