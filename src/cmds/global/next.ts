...
export async function run(interaction: CommandInteraction): Promise<unknown> {
	...
	const fiep = `S${normalize(interaction.options.data[0]?.options?.filter((x) => x.name === 'season')?.[0].value as number)}E${normalize(
		interaction.options.data[0]?.options?.filter((x) => x.name === 'episode')?.[0].value as number
	)}`;
	...
	const GetNextEmbed = (show: string, season_and_episode: string) => {
		...
		if (!f) return new MessageEmbed().setTitle(`${show} ${season_and_episode} is not a valid episode!`);

		const next = shows[shows.indexOf(f) + 1];
		if (!next) return new MessageEmbed().setTitle("Couldn't find that episode. Try again.");
