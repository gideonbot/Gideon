...
export async function run(interaction: CommandInteraction): Promise<unknown> {
	...
	const body = (await fetchJSON(api)) as TVMazeResponse;

	if (body.status === 404) return interaction.editReply({ embeds: [new MessageEmbed().setTitle('There was no data for this episode!')] });
