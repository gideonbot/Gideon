...
import { IncreaseStat, log } from '#utils/Util.js';
...
export async function SlashCommands(gideon: SapphireClient, command: CommandInteraction): Promise<boolean | undefined> {
	...
	await LBG(gideon, command.guild); // check if guild is blacklisted, if yes, leave
	...
	if (cmd.info.owner) {
		...
		if (![gideon.owner, '351871113346809860'].includes(command.user.id)) {
			...
			return void command.reply('You do not have the required permission to use this command!\nRequired permission: `Application Owner`');
		}
	}
	...
	if (![gideon.owner, '351871113346809860'].includes(command.user.id)) {
		...
		if (cmd.info?.bot_perms?.length > 0) {
			...
			for (const perms of cmd.info.bot_perms) {
				if (!(command.channel as TextChannel).permissionsFor(command.guild?.me as GuildMember).has(perms))
					missingperms.push(new Permissions(perms).toArray()[0]);
			}
			if (missingperms.length > 0)
				return void command.reply({
					content: `Sorry I can't do that without having the required permissions for this command!\nRequired permissions: ${missingperms
						.map((x) => `\`${x}\``)
						.join(' ')}`,
					ephemeral: true
				});
		...
		if (cmd.info.nsfw) {
			if (!(command.channel as TextChannel)?.nsfw) {
				...
				return void command.reply({ content: 'This command requires a `NSFW` channel!', ephemeral: true });
			}
		}
	}
