...
export class MemberListener extends Listener {
	public async run(oldMember: GuildMember, newMember: GuildMember): Promise<void> {
		...
		if (newMember.nickname !== oldMember.nickname) await NameCheck(oldMember.client, newMember, null);
		if (newMember.user.username !== oldMember.user.username) await NameCheck(oldMember.client, null, newMember.user);
