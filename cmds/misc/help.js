    ...
    if (Object.keys(commands).length > 10) {
        ...
        for (let i = 0; i < arrs.length; i++) {
            ...
            for (let item of arrs[i]) {
                ...
                embed.addField(commands[item].help_text.toLowerCase().startsWith("gideon") || commands[item].help.type === 'voice' ? item + ` ${marks[item].emotes.join('')}${marks[item].roles.length > 0 ? '\`' + marks[item].roles.join(' ') + '\`' : ''}` : customprefix.prefix + item + ` ${marks[item].emotes.join('')}${marks[item].roles.length > 0 ? '\`' + marks[item].roles.join(' ') + '\`' : ''}`, commands[item].help_desc);
            }
