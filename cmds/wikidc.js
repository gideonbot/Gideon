...
module.exports.run = async (gideon, message, args) => {
    ...
    const search_api = encodeURI(`https://dc.fandom.com/api/v1/SearchSuggestions/List?query=${search_term}`);
    ...
    try {
        ...
        const api = encodeURI(`https://dc.fandom.com/api/v1/Articles/Details?ids=50&titles=${search_term}&abstract=500&width=200&height=200`);

        const body = await fetch(api).then(res => res.json());
        const article = Object.values(body.items)[0];       
        if (!Object.keys(body.items).length) return message.channel.send(`There was no result for ${search_term} on the DC Wiki!`).catch(console.error);
        const url = article.url.replace(/\(/g, '%28').replace(/\)/g, '%29');       
                
        const wikiart = new Discord.MessageEmbed()
        .setColor('#2791D3')
        .setTitle(article.title)
        .setDescription(article.abstract + `\n\n**[Click here to read the full article](https://dc.fandom.com${url} 'https://dc.fandom.com${url}')**`)
